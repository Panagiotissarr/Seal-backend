...
import { getState, updateState, subscribeToState, type BotState } from './services/botState.js';
...
async function syncStateFromDb() {
  ...
  if (currentState) {
    console.log(`State loaded: status=${currentState.status}, kill=${currentState.kill_switch}`);
    if (currentState.status === 'error') {
      console.log('Resetting stale error state to offline');
      await updateState({ status: 'offline', bot_id: null, bot_username: null });
      currentState = { ...currentState, status: 'offline', bot_id: null, bot_username: null };
    }
  }
}
...
async function main() {
  ...
  startDiscordBot().then(async (success) => {
    console.log(`Discord bot auto-start: ${success ? 'connected' : 'failed (will retry on demand)'}`);
  ...
  subscribeToState(async (newState) => {
    ...
    if (newState.status === 'online' && !client.isReady() && !newState.kill_switch) {
      ...
      client.user?.setActivity();
    } else if (!newState.kill_switch && client.isReady() && newState.custom_status) {
  ...
  app.listen(PORT, () => {
    ...
    console.log(`API auth: ${process.env.API_KEY ? 'enabled' : 'disabled (set API_KEY for security)'}`);
  });
}
