...
export async function handleImageSearch(query: string): Promise<string | null> {
  try {
    ...
    if (data.results?.length > 0) {
      ...
      return (
        data.results[Math.floor(Math.random() * maxIdx)].image ||
...
export async function handleImageGeneration(
  ...
  try {
    ...
    const res = await fetch(
      `${process.env.CLOUD_API_URL || "https://fpxtucteydpmuswitebs.supabase.co/functions/v1/api"}/generate`,
      {
        method: "POST",
        headers: {
...
export async function handleWebSearch(
  ...
  try {
    const res = await fetch(
      `${process.env.CLOUD_API_URL || "https://fpxtucteydpmuswitebs.supabase.co/functions/v1/api"}/search`,
      {
        method: "POST",
...
export async function storeUserMessage(
  ...
  await supabase
    .from("bot_memories")
    .insert({ user_id: userId, username, key, value: content.slice(0, 500) });
...
export async function storeMemory(
  ...
  if (existing && existing.length > 0) {
    ...
    await supabase
      .from("bot_memories")
      .insert({ user_id: userId, username, key, value });
  }
}
...
export function parseMemoryStore(
  text: string,
  ...
  const kvMatch = text.match(
    /remember (?:that |the )?(?:(?:my |the )?(\w[\w\s]*?) (?:is|are|=) (.+))/i,
  );
  if (kvMatch)
    return { key: kvMatch[1].trim().toLowerCase(), value: kvMatch[2].trim() };

  const genericMatch = text.match(/remember (.+)/i);
  if (genericMatch)
    return { key: `memo_${Date.now()}`, value: genericMatch[1].trim() };
...
export function needsWebSearch(text: string): string | null {
  ...
  for (const pattern of searchPatterns) {
    ...
    if (
      ...
      !["i", "you", "we", "they", "it", "that", "this", "me", "us"].includes(
...
export function extractUserQuery(text: string): string | null {
  ...
  for (const p of patterns) {
    ...
    if (
      match?.[1] &&
      !["i", "you", "we", "they", "it", "that", "this", "someone"].includes(
        match[1].toLowerCase(),
      )
    ) {
      return match[1];
