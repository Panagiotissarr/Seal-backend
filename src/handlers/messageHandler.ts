...
export async function handleMessage(message: Message, state: BotState) {
  ...
  const hasAttachments = message.attachments.some((a) =>
    ...
    if (!isNaN(num) && num > 0) {
      if (num === expectedNext) {
        ...
        if (milestones.includes(num)) {
          ...
          await (message.channel as any).send(
            celebrations[num] || `🎉 Milestone: **${num}**! ARF ARF! 🦭`,
          );
        }
        return;
      } else {
        ...
        await updateState({
          counting_number: 1,
          counting_last_user: client.user?.id || "seal_bot",
        });
        return;
      }
    }
  ...
  // Direct year/name question handling
  const lc = rawContent.toLowerCase().trim();
  if (
    /what year is it|current year|what is the year|tell me the year|what year are we in/i.test(
      lc,
    )
  ) {
    const y = new Date().getFullYear();
    await (message.channel as any).send({
      content: `Arf arf! The current year is **${y}**! *checks tiny seal calendar* 🦭📅`,
      reply: { messageReference: message.id },
    });
  ...
  if (
    /what is your name|what's your name|who are you|what are you|tell me your name|do you have a name|whats your name/i.test(
      lc,
    )
  ) {
    await (message.channel as any).send({
      content: "Arf arf! I dont have a name but you can call me seal 🦭",
      reply: { messageReference: message.id },
    });
    await updateState({ messages_sent: (state.messages_sent || 0) + 1 });
    return;
  ...
  const defaultPrompt =
    getDefaultPrompt() ||
    `You are a cute seal. Reply in character (1-3 sentences). Respond in seal character:`;
  const systemPrompt =
    state.personality ||
    `${defaultPrompt}\n\n${extraContext}\n\n${userMemories}`;
  ...
  try {
    ...
    const reply =
      data.answer ||
      "Arf! *tilts head* I have no idea what to say about that! 🦭";
